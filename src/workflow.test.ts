...
): Pick<ChapterStage, "stage" | "status">[] =>
  ["RAW", "CLEAN_REDRAW", "TRANSLATION", "TYPESET", "REVIEW", "READY"].map(
    (stage) => ({
      stage: stage as ChapterStage["stage"],
      status: done.includes(stage) ? "COMPLETED" : "WAITING",
    }),
  );
describe("scan workflow", () => {
  it("releases clean and translation after raw, then type only after both", () => {
    ...
    expect(isStageAvailable("TYPESET", stages(["RAW", "CLEAN_REDRAW"]))).toBe(
      false,
    );
    expect(
      isStageAvailable(
        "TYPESET",
        stages(["RAW", "CLEAN_REDRAW", "TRANSLATION"]),
      ),
    ).toBe(true);
  });
  it("releases review and ready in sequence", () => {
    expect(
      nextStatuses("TYPESET", stages(["RAW", "CLEAN_REDRAW", "TRANSLATION"])),
    ).toContain("REVIEW");
    expect(
      isStageAvailable(
        "READY",
        stages(["RAW", "CLEAN_REDRAW", "TRANSLATION", "TYPESET", "REVIEW"]),
      ),
    ).toBe(true);
  });
  it("permits exactly one reservation for an available stage", () => {
    ...
    expect(() => reserveStage(henrique, "maria")).toThrow(
      "acabou de ser assumida",
    );
  });
