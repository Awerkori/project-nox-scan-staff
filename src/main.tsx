...
function App() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: chapterError } = await supabase.from('chapters').select('id,number,title,work:works(id,title),chapter_stages(id,chapter_id,stage,status,assigned_to,assigned_at,completed_at,assignee:profiles!chapter_stages_assigned_to_fkey(display_name,github_login,avatar_url))').order('created_at', { ascending: false }).limit(100)
  ...
  return <HashRouter><PanelRoutes member={member} chapters={chapters} notifications={notifications} toast={toast} refresh={() => void load()} logout={() => void supabase?.auth.signOut()} /></HashRouter>
}
