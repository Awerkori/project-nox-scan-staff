...
function App() {
  ...
  const load = useCallback(async () => {
    ...
    setMember({ user_id: staff.user_id, github_login: staff.github_login, display_name: staff.display_name, is_admin: staff.is_admin, roles: (staff.user_roles ?? []).flatMap((item: { roles: { code: Role } | null }) => item.roles ? [item.roles.code] : []) })
...
function Task({ chapter, stage, member, onChanged }: { chapter: Chapter; stage: ChapterStage; member: StaffMember; onChanged: () => void }) {
  ...
  const perform = async (action: 'claim' | 'release') => { try { action === 'claim' ? await claimStage(stage.id) : await releaseStage(stage.id); onChanged() } catch (cause) { setMessage(cause instanceof Error ? cause.message : 'Não foi possível atualizar a tarefa.') } }
